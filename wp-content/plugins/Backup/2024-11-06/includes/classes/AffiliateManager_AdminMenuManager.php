...
class AffiliateManager_AdminMenuManager
{
     ...
     */
    public function render_campaigns_page_OLD()
    {
        $template = plugin_dir_path(__FILE__) . '../../templates/campaign-management.php';
        if (file_exists($template)) {
            include $template;
        } else {
            echo '<div class="error"><p>' . esc_html__('Error: Template file not found for managing campaigns. Path: ', 'affiliate-manager') . esc_html($template) . '</p></div>';
        }
    }
